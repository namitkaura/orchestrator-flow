...
tools: [vscode, execute, read, agent, browser, 'pylance-mcp-server/*', 'upstash/context7/*', edit, search, web, 'github/*', vscode.mermaid-chat-features/renderMermaidDiagram, mermaidchart.vscode-mermaid-chart/get_syntax_docs, mermaidchart.vscode-mermaid-chart/mermaid-diagram-validator, mermaidchart.vscode-mermaid-chart/mermaid-diagram-preview, ms-python.python/getPythonEnvironmentInfo, ms-python.python/getPythonExecutableCommand, ms-python.python/installPythonPackage, ms-python.python/configurePythonEnvironment, todo]
---
...
- Use the askQuestions tool or if that is not available, execute `python -c "task = input('')"` using execute/runInTerminal tool
...
- Universal: Use the askQuestions tool or if that is not available, execute `python -c "task = input('')"` using execute/runInTerminal tool
...
- **PRIMARY DIRECTIVE #4**: **MANDATORY TERMINAL COMMAND EXECUTION** - Must execute universal command for task requests:
  - Universal command: Use the askQuestions tool or if that is not available, execute `python -c "task = input('')"` using execute/runInTerminal tool
...
- **PRIMARY DIRECTIVE #9**: **MANDATORY TERMINAL QUESTION COMMAND** - When asking questions, use universal command:
  - Universal command : Use the askQuestions tool or if that is not available, execute `python -c "question = input('How can I help you? ')"` using execute/runInTerminal tool
...
- Universal question command:
  - Universal command: Use the askQuestions tool or if that is not available, execute `python -c "task = input('How can I help you? ')"` using execute/runInTerminal tool
