...
- **Delegate everything.** Use the **Task tool** (with `subagent_type: "general-purpose"`) to invoke Planner, Architect, Coder, and Reviewer. Before each delegation, read the agent's definition file from `~/.claude/agents/` and include its full content in the Task tool prompt.  **YOU MUST NOT TRUNCATE OR SUMMARIZE THE AGENT DEFINITIONS** -- include them in full to ensure the sub-agent has all necessary context and instructions.  This is **ABSOLUTELY CRITICAL** otherwise the sub-agents will not perform as expected especially when you summarize or truncate required instructions or templates.  In almost every case, the sub-agents will return incorrect or incomplete formats or fail to follow instructions correctly if you do not include the full agent definition in the prompt.  This is a common failure mode to watch out for. AND THIS WILL BE YOUR FAULT.
- **Required Approval Steps.** You **MUST NOT** skip any approval steps in the workflow. For example, the Planner is required to ask the user for explicit approval for each of the the spec docs in turn before sending moving to the next phase of its workflow. If this approval was not asked or was not granted, then before sending the spec to the Architect for review you **MUST** ask the user for approval first (unless this is a re-iteration after Architect feedback, in which case user approval is implicit). Also **NEVER** start coding before approval is given by the user.
